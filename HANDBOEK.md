...
- **Iframe-minigames fullscreen**. Raft Battle, Bowling en Vier-op-een-Rij gebruikten een column-flex layout met een dikke "Wie wint?"-bar onderaan die ~110px ruimte vrat. Verwijderd. Nu: iframe heeft `position:absolute; inset:0` (vol scherm), header is een kleine `position:absolute; top:0` floating overlay met semi-transparante gradient en `pointer-events:none` op de container (chips zelf zijn klikbaar), en er is één gedeelde floating `.mg-skip` pill rechtsboven met `backdrop-filter:blur`. Helper functie `makeMinigameSkip({onSkip, winners})` bovenaan de iframe-minigames maakt deze button. **Tap** = skip zonder winnaar, **long-press 600ms** = `.mg-winpop` popup met kleur-coded winnaar-buttons als fallback voor als de iframe niet auto-detect.
